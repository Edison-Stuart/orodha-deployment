...
def test_get_user_from_access_token(
    mock_create_client_connection,
    mock_create_admin_connection
):
    connection = orodha_keycloak.connection.KeycloakConnection(
    ...
    user = connection.get_user_from_access_token(
        {"access_token": "data"}, is_token=True)

    assert user == MOCK_DATA["mock_decoded_token"]


def test_get_user_from_access_token_id(
    mock_create_client_connection,
    mock_create_admin_connection
):
    connection = orodha_keycloak.connection.KeycloakConnection(
    ...
    user = connection.get_user_from_access_token("someid", is_token=False)
